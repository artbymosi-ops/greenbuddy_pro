...
import Layout from "@/components/Layout";
import Plant2D from "@/components/Plant2D"; // <— nový 2D komponent
        ...
        <section className="card" style={{ marginBottom: 16 }}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button className={`btn ${mode==="2d" ? "" : "ghost"}`} onClick={()=>setMode("2d")}>🌿 2D</button>
              <button className={`btn ${mode==="3d" ? "" : "ghost"}`} onClick={()=>setMode("3d")}>🌱 3D</button>
            </div>
          </div>
