...
export default function Calendar(){return(<Layout title="Kalender"><div className="card"><p className="subtitle">Kalender kommt gleich – pridáme Supabase Events.</p></div></Layout>);}
...
export default function CalendarPage(){
  ...
  const [events, setEvents] = useState([]);   // reminders
  ...
  // --- SUPABASE SYNC (ak je k dispozícii + user) ---
  useEffect(()=>{
    let sub1, sub2;
    (async ()=>{
      if(!supabase) return;
    ...
    if(supabase){
      ...
      if(user){
        const { data, error } = await supabase.from("calendar_events").insert({
          user_id:user.id, plant_id: ev.plant_id, title:ev.title, note:ev.note, type:ev.type,
          date:new Date(ev.date).toISOString()
        }).select().single();
        if(!error && data){ setEvents(prev=>[...prev,data]); scheduleLocalNotify(data); return; }
      }
    }
    const local = { ...ev, _localId: crypto.randomUUID() };
  ...
  useEffect(()=>{
    ...
    // po refreshe naplánuj blízke udalosti
    const now = Date.now();
    events.forEach(ev => {
  ...
  function scheduleLocalNotify(ev){
    ...
    if(diff <= 0 || diff > 24*60*60*1000) return; // len do 24h
  ...
  return (
    <Layout>
      ...
      <CalendarMonth plants={plants} events={events} onAdd={add} onDelete={del}/>
    </Layout>
  );
       }
