...
export default function DiaryPage() {
  const [plants, setPlants] = useState([]);       // z /plants
  const [entries, setEntries] = useState([]);     // z DB / local
  ...
  // --- Načítaj rastliny (local + supabase) ---
  useEffect(() => {
    // local
    try {
      const p = JSON.parse(localStorage.getItem("gb_plants") || "[]");
      setPlants(p);
    } catch {}
    // supabase
    (async () => {
  ...
  useEffect(() => {
    try {
      const d = JSON.parse(localStorage.getItem("gb_diary") || "[]");
      setEntries(d);
    } catch {}
  }, []);
  useEffect(() => {
    localStorage.setItem("gb_diary", JSON.stringify(entries));
  }, [entries]);
  ...
  // upload fotiek (Supabase → URL) / fallback (dataURL)
  async function uploadPhotos(filesArr) {
    if (!filesArr?.length) return [];
    // Supabase bucket: "diary"
    if (supabase) {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        const urls = [];
    ...
    // fallback – DataURL (pozor na veľkosť)
  ...
  async function addEntry(e) {
    ...
    const payload = {
      date,                                          // YYYY-MM-DD
      plant_id: typeof plantId === "number" ? plantId : null,
  ...
  const filtered = useMemo(
    () => entries.filter(e => filterPlant === "all" ? true : (String(e.plant_id ?? "") === String(filterPlant))),
    [entries, filterPlant]
  );
        ...
        <form onSubmit={addEntry} className="ev-form" style={{marginTop:12}}>
          <div className="grid grid-3" style={{gap:12}}>
            ...
            <div>
              ...
              <input className="input" type="number" min="0" step="0.5"
                     value={height} onChange={e=>setHeight(e.target.value)} placeholder="z. B. 42" />
            </div>
          </div>
          ...
          <div className="grid" style={{gap:12, marginTop:12}}>
            <div>
              ...
              <input className="input" value={plantName} onChange={e=>{ setPlantName(e.target.value); setPlantId(""); }}
                     placeholder="z. B. Monstera im Büro" />
            </div>
            <div>
              <label className="subtitle">Fotos (Vorher/Nachher möglich)</label>
              <input className="input" type="file" multiple accept="image/*"
                     onChange={e=>setFiles([...e.target.files])}/>
              {files?.length>0 && <div className="subtitle" style={{marginTop:4}}>
          ...
          <textarea className="input" rows={4} value={note} onChange={e=>setNote(e.target.value)}
                    placeholder="Wie geht’s der Pflanze? Pflege, Standort, Beobachtungen …" />

          <button className="btn" type="submit" disabled={saving}
                  style={{marginTop:12}}>{saving ? "Speichere…" : "Eintrag hinzufügen"}</button>
        </form>
      </div>
      ...
      <div className="card" style={{marginTop:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <select className="input" style={{maxWidth:260}}
                  value={filterPlant} onChange={e=>setFilterPlant(e.target.value)}>
                ...
                {Array.isArray(entry.photos) && entry.photos.length > 0 && (
                  <div className="grid" style={{gap:8, marginTop:10}}>
                    {entry.photos.map((url, i)=>(
                      <img key={i} src={url} alt="Foto" style={{
                        width:"100%", maxWidth:220, aspectRatio:"1 / 1",
                        objectFit:"cover", borderRadius:12, border:"1px solid rgba(255,255,255,0.06)"
                      }}/>
                    ))}
                  </div>
                )}
