    ...
    <Layout>
      <div className="card">
        ...
        <p className="subtitle">
          Füge deine echten Pflanzen hinzu. Diese kannst du im Kalender für Erinnerungen auswählen.
        </p>

        <form onSubmit={addPlant} className="ev-form" style={{ marginTop: 12 }}>
          <input
            className="input"
            placeholder="Name (z. B. Monstera im Wohnzimmer)"
            value={form.name}
            onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
          />
          <input
            className="input"
            placeholder="Art (optional, z. B. Monstera deliciosa)"
            value={form.species}
          ...
          <li key={p.id ?? p._localId}>
            <div>
              <div className="ev-title">
                <span className="ev-tag" style={{ background: "#5bbf7a" }}>
                  {p.emoji || "🌿"}
                </span>
                {p.name}
              </div>
              {p.species && (
                <div className="subtitle" style={{ marginTop: 2 }}>{p.species}</div>
              )}
            </div>
            <button
              className="btn ghost"
              onClick={() => removePlant(p.id ?? p._localId)}
            >
              Löschen
            </button>
          </li>
