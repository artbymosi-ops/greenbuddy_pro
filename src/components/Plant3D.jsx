...
/** Plant3D – reaguje na mood/level/lastAction
...
export default function Plant3D({ state, lastAction }) {
  ...
  const config = useMemo(() => {
    const leaves = 2 + Math.floor(level * 0.8);
    const fen = level >= 3;              // od 3. levelu dierky v listoch
    const scale = 0.95 + level * 0.06;
  ...
  return (
    <div className="plant-wrap">
      <svg className="scene" viewBox="0 0 400 280" width="100%" height="auto">
          ...
          return (
            <g key={i} transform={`scale(${config.scale})`} style={{ transformOrigin: "200px 200px" }}>
              ...
              >
                ...
                <path
                  d={`M${cx} ${cy} c0 ${r * 0.5} -${r * 0.35} ${r * 0.6} -${r * 0.6} ${r * 0.7}`}
                  stroke={vein}
                  strokeWidth="4"
                  fill="none"
                  opacity=".55"
        ...
        <g transform="translate(0,10)">
          ...
          <path
            d={
              mood === "sad"
                ? "M170 206 q20 -14 40 0"
                : mood === "happy"
                ? "M170 200 q20 12 40 0"
                : "M170 202 q20 0 40 0"
            }
            stroke="#121518"
            strokeWidth="5"
      ...
      <style jsx>{`
        ...
        .leaf.shimmer{filter:brightness(1.2)}
